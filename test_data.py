...
def create_users(db):
    ...
    def process_users(jmena_list, role_objs):
        for jmeno in jmena_list:
            ...
            # Kontrola duplicit v rámci této dávky (zjednodušené)
            if any(u.login == login for u in created_users):
                login += "2"

            # Kontrola v DB
            if db.query(Users).filter_by(login=login).first():
                # Načteme existujícího, abychom ho mohli vrátit
            ...
            # Přiřazení rolí (SQLAlchemy magic)
...
# --- KROK 3: Vína a Hodnocení ---
def create_wines_and_ratings(db, rocniky_map, users):
    # Filtrace uživatelů podle rolí v Pythonu (protože máme načtené objekty)
    # Pozor: u.roles je seznam objektů Role
    ...
    for rok, rocnik_obj in rocniky_map.items():
        ...
        # Náhodný výběr aktivních vinařů pro tento ročník
        ...
        for vinar in current_vinari:
            ...
            for _ in range(pocet_vin):
                ...
                # Logika generování vína (zachována z původního skriptu)
